...
public class BindSettingComponent extends SettingComponent {
    ...
    Supplier<String> descriptionText = Suppliers.memoize(() -> MsdfUtil.cutString(getSettingLayer().getDescription().getString(), 6, getTextColumnWidth()));
    ...
    public BindSettingComponent render(DrawContext context, int mouseX, int mouseY, float delta) {
        ...
        String valueText = KeyBoardUtil.translate(bindSetting.get().getKey());
        float valueTextWidth = QuickApi.inter().getWidth(valueText, 6);
        float valueX = getValueColumnX();
        float radius = InterfaceStyle.radius(2f, 1f);
        ...
        QuickApi.border()
                .size(new SizeState(VALUE_COLUMN_WIDTH, 9))
        ...
        QuickApi.text()
                ...
                .font(QuickApi.inter())
                .build()
                .render(context.getMatrices().peek().getPositionMatrix(), valueX + (VALUE_COLUMN_WIDTH - valueTextWidth) / 2f, getY() + 1);
