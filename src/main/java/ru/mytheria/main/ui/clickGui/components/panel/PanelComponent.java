...
public class PanelComponent extends Component {
    ...
    @Override
    public void init() {
        componentsList = Helper.moduleLayers(category, (e) -> {
            String query = SearchComponent.getSearchSource().get().getText().toString().toLowerCase();
            String name = e.getModuleName().getString().toLowerCase();
    ...
    public PanelComponent render(DrawContext context, int mouseX, int mouseY, float delta) {
        animationScroll = MathHelper.lerp(.35f, animationScroll, scroll);
    ...
    public boolean mouseClicked(double mouseX, double mouseY, int button) {
        if (backgroundComponent.mouseClicked(mouseX, mouseY, button)) {
            return true;
        }
