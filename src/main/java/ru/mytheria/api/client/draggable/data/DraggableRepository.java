...
import ru.mytheria.main.ui.elements.draggable.ArmorDraggable;
import ru.mytheria.main.ui.elements.draggable.CooldownsDraggable;
import ru.mytheria.main.ui.elements.draggable.InventoryDraggable;
import ru.mytheria.main.ui.elements.draggable.KeybindsDraggable;
import ru.mytheria.main.ui.elements.draggable.PotionsDraggable;
...
public class DraggableRepository implements QuickImport {
        ...
        list.forEach(e -> {
            ...
            Math.scale(context.getMatrices(), e.getX() + e.getWidth() / 2, e.getY() + e.getHeight() / 2, e.getAnimation().getOutput().floatValue(), () -> {
                ...
                if (mc.currentScreen instanceof ChatScreen) {
                    renderResetHandle(context, e, mouseX, mouseY);
                    renderResizeHandle(context, e, mouseX, mouseY);
                }
            });
        });
    }
    ...
    public void update(DrawContext context, float tickDelta, int mouseX, int mouseY) {
        ...
        java.util.Optional<Draggable> resizing = list.stream()
                .filter(Draggable::getResizing)
                .findFirst();

        if (resizing.isPresent()) {
            Draggable draggable = resizing.get();
            float deltaX = mouseX - draggable.getResizeStartMouseX();
            float deltaY = mouseY - draggable.getResizeStartMouseY();
            float delta = (deltaX + deltaY) / 2f / 120f;
            draggable.setSizeScale(MathHelper.clamp(draggable.getResizeStartScale() + delta, 0.7f, 1.5f));
            return;
        }
    ...
    public void mouseClicked(double mouseX, double mouseY, int button) {
        list.forEach(e -> {
            ...
            if (Math.isHover(mouseX, mouseY, e.getX(), e.getY(), e.getWidth(), e.getHeight())) {
                if (button == 0 && mc.currentScreen instanceof ChatScreen && e.isResetHandleHovered(mouseX, mouseY)) {
                    e.resetSizeScale();
                    saveState();
                    return;
                }

                if (button == 0 && mc.currentScreen instanceof ChatScreen && e.isResizeHandleHovered(mouseX, mouseY)) {
                    e.beginResize((float) mouseX, (float) mouseY);
                    return;
                }
    ...
    public void mouseReleased(double mouseX, double mouseY, int button) {
        ...
        for (Draggable e : list) {
            if (!e.getVisible().get() && !(mc.currentScreen instanceof ChatScreen)) {
                continue;
            }

            if (button == 0 && e.getDragging()) {
                e.setDragging(false);
                shouldSave = true;
            }

            if (button == 0 && e.getResizing()) {
                e.setResizing(false);
                shouldSave = true;
            }
        }
    ...
    public void saveState() {
        java.io.File file = getStateFile();
        ...
        for (Draggable draggable : list) {
            ...
            draggableObject.addProperty("scale", draggable.getSizeScale());
            root.add(draggable.getClass().getSimpleName(), draggableObject);
        }
    ...
    private void loadState() {
        ...
        try (java.io.Reader reader = new java.io.FileReader(file)) {
            ...
            for (Draggable draggable : list) {
                JsonObject draggableObject = root.getAsJsonObject(draggable.getClass().getSimpleName());
                if (draggableObject == null) {
                    continue;
                }

                if (draggableObject.has("x")) {
                    draggable.setX(draggableObject.get("x").getAsFloat());
                }

                if (draggableObject.has("y")) {
                    draggable.setY(draggableObject.get("y").getAsFloat());
                }

                if (draggableObject.has("scale")) {
                    float scale = draggableObject.get("scale").getAsFloat();
                    draggable.setSizeScale(MathHelper.clamp(scale, 0.7f, 1.5f));
                }
            }
        } catch (java.io.IOException ignored) {
        }
    ...
    private java.io.File getStateFile() {
        return new java.io.File(mc.runDirectory.getAbsolutePath() + "/mytheria/draggables.json");
    }

    private void renderResizeHandle(DrawContext context, Draggable draggable, double mouseX, double mouseY) {
        float size = draggable.getResizeHandleSize();
        float x = draggable.getResizeHandleX();
        float y = draggable.getY() + draggable.getHeight() - size - 2f;
        int fillColor = draggable.getResizing() || draggable.isResizeHandleHovered(mouseX, mouseY)
                ? 0xCFFFFFFF
                : 0x85FFFFFF;

        QuickApi.rectangle()
                .size(new SizeState(size, size))
                .radius(new RadiusState(1.8f))
                .color(new ColorState(fillColor))
                .build()
        ...
        QuickApi.border()
                ...
                .radius(new RadiusState(1.8f))
                .thickness(-2.2f)
                .color(new ColorState(0xFF000000))
                .build()
                .render(context.getMatrices().peek().getPositionMatrix(), x, y);
    }

    private void renderResetHandle(DrawContext context, Draggable draggable, double mouseX, double mouseY) {
        float size = draggable.getResizeHandleSize();
        float x = draggable.getResetHandleX();
        float y = draggable.getY() + draggable.getHeight() - size - 2f;
        int fillColor = draggable.isResetHandleHovered(mouseX, mouseY)
                ? 0xCFFFFFFF
                : 0x85FFFFFF;
        ...
        context.drawTextWithShadow(mc.textRenderer, "↺", (int) (x + 1.0f), (int) (y - 1.0f), 0xFF000000);
    }
}
